...
class DeltaGripper:
    def __init__(self, init=True):
        ...
        self.reset_pub = rospy.Publisher('/delta_gripper/reset', Empty, queue_size=2)
        self.stop_pub = rospy.Publisher('/delta_gripper/stop', Bool, queue_size=2)
        self.delta_position_trajectory_pub = rospy.Publisher('/delta_gripper/joint_position_trajectory', Float32Array, queue_size=2);
        self.delta_velocity_trajectory_pub = rospy.Publisher('/delta_gripper/joint_velocity_trajectory', Float32Array, queue_size=2);
    ...
    def wait_until_done_moving(self):
        ...
        while not done_moving:
            try:
                done_moving_deltas_msg = rospy.wait_for_message('/delta_gripper/done_moving_deltas', Bool, timeout=11)
